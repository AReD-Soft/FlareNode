#!/system/bin/sh
# service.sh: Auto-start & Manual trigger Flare Node daemon
...
if [ -f "$MODDIR/token.txt" ]; then
    ...
    if [ -n "$TOKEN" ]; then
        ...
        log "[service] Starting Flare Node tunnel..."
        ...
        log "[service] Daemon active in background."
        echo "Flare Node daemon started."
    else
        log "[service] Token empty, skipping execution."
        echo "Error: Token empty."
    fi
else
    log "[service] Token file not found!"
